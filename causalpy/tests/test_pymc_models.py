...
def test_etwfe_regression_predict_returns_in_sample(mock_pymc_sample):
    ...
    # mu carries dims (chain, draw, obs_ind, treated_units), so score() and the
    # downstream impact machinery keep working off this in-sample prediction.
    scores = model.score(panel["X"], panel["y"])
    assert isinstance(scores, pd.Series)
    assert prediction["posterior_predictive"]["mu"].dims[-2] == "obs_ind"
