...
class TestParser:
    ...
    @pytest.mark.parametrize(
        "formula",
        [
            "y ~ x1 + (1 + I(x1 ** 2) | store_id)",
            "y ~ x1 + (1 | C(store_id))",
        ],
    )
    def test_parse_rejects_unrecognized_random_effects_syntax(self, formula):
        """Raises a parser-owned error for unsupported random-effect expressions."""
        with pytest.raises(ValueError, match="Could not parse random-effects term"):
            Parser.parse(formula)

    def test_parse_allows_nested_fixed_effect_expression(self):
        """Keeps nested expressions valid when they occur outside random terms."""
        parsed = Parser.parse("y ~ I(x1 ** 2) + (1 + x1 | store_id)")

        assert parsed.rhs == "I(x1 ** 2)"
        assert parsed.random_components[0].formula_rhs == "1 + x1"
...
class TestMixedModelFormula:
    ...
    def test_random_effect_intercept_conventions(
        self, formula, mixed_effect_model_data
    ):
        """Includes a random intercept unless the random term removes it."""
        matrices = parse_formula(formula, mixed_effect_model_data)
    ...
    def test_random_effect_no_intercept_conventions(
        self, formula, mixed_effect_model_data
    ):
        """Excludes the random intercept for standard no-intercept conventions."""
        matrices = parse_formula(formula, mixed_effect_model_data)

        assert list(matrices.Z.columns) == ["x1|store_id"]
