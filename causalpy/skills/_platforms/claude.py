...
"""Claude Code platform adapter — writes skills to ``.claude/skills/causalpy-<name>/``."""
...
class ClaudeAdapter(BasePlatformAdapter):
    """Install/uninstall CausalPy skills for Claude Code."""

    @property
    def _skills_root(self) -> Path:
        """Return the Claude Code skills directory path."""
        return self.project_dir / ".claude" / "skills"

    def install(self, skills_data: dict[str, dict[str, str]]) -> list[Path]:
        """Write each skill to ``.claude/skills/causalpy-<name>/``."""
        written: list[Path] = []
