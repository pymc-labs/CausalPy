...
"""Cursor platform adapter — writes skills to ``.cursor/skills/causalpy-<name>/``."""
...
class CursorAdapter(BasePlatformAdapter):
    """Install/uninstall CausalPy skills for Cursor."""

    @property
    def _skills_root(self) -> Path:
        """Return the Cursor skills directory path."""
        return self.project_dir / ".cursor" / "skills"

    def install(self, skills_data: dict[str, dict[str, str]]) -> list[Path]:
        """Write each skill to ``.cursor/skills/causalpy-<name>/``."""
        written: list[Path] = []
