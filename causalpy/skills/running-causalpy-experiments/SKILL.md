...
6. Inspect outputs with `summary()`, `effect_summary()`, `print_coefficients()`, and `plot()` only after `fit()`; results live on `exp.result` and `exp.prior_result`, and read methods raise `GroupNotSampleedException` naming the missing call when a phase has not run.
