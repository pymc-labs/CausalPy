...
Use this guide as a lookup appendix after reading [Decision tree](decision_tree.md). Do not use these tables as the primary route; the canonical selection path is ordered by assignment mechanism, data topology, controls, disqualifiers, and output constraints.
...
| Synthetic-control setting with unit weights and pre-period time weights | `SyntheticDifferenceInDifferences` | Wide panel, columns are units | Weighted controls and weighted pre-periods provide a credible counterfactual | `SyntheticControl` for simpler donor-weight-only analysis; treat SDiD as Bayesian-only in practice |
...
- Does the requested method fall outside CausalPy, such as matching, fuzzy RD, non-absorbing staggered treatment, augmented synthetic control, mediation, survival models, or CATE discovery?
