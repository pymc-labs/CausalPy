...
class RandomComponent:
    ...
    def formula_rhs(self) -> str:
        ...
        rhs_terms = ["1"] if self.has_intercept else ["0"]
...
class Parser:
    ...
    def parse(cls, formula: str) -> MixedModelFormula:
        ...
        if "|" in fixed_rhs:
            raise ValueError(
                "Could not parse random-effects term in "
                f"{formula!r}. Expected '(intercept + slopes | grouping_variable)'."
            )
