...
class StaggeredDifferenceInDifferences(BaseExperiment):
    ...
    With ``estimator="etwfe"`` a fourth assumption is added:

    4. **Correct specification of the saturated design**: the extended two-way
       fixed effects estimator fits the *full sample*, so the untreated
       potential outcome model (unit effects, time effects, any covariates) and
       the treatment effect surface are estimated jointly. Every treated cell
       must therefore be represented by its own ``(cohort, event-time)``
       column, and anything the design omits -- a unit-specific trend, an
       interaction between a covariate and the treatment effect -- is absorbed
       into the effect estimates. The imputation estimator carries a weaker
       burden here: it fits the outcome model on untreated cells only, so a
       mis-specified treatment effect surface cannot contaminate the
       counterfactual.
    ...
    **The ATT is unaffected.** ``tau`` -- and hence ``att_`` -- is identified off
    ...
    Regression, and Difference-in-Differences Estimators. SSRN Electronic
    Journal.
    ...
    supports_ols = True
    supports_bayes = True
    _default_model_class: type[PyMCModel] | None = LinearRegression
    ...
    def _build_etwfe_design_ols(self) -> None:
        ...
        y, X = dmatrices(self.etwfe_formula_, self.data)
        self._y_design_info = y.design_info
        self._x_design_info = X.design_info
        self.labels = X.design_info.column_names
        self.X_full = np.asarray(X)
    ...
    def _fit_model_etwfe(self) -> None:
        ...
        if isinstance(model, ETWFERegression):
            ...
            # ``_etwfe_fitted`` is deliberately separate from ``y_pred``: the OLS
            # ETWFE fitted values are a plain array while ``y_pred`` carries an
            # InferenceData on the Bayesian path.
    ...
    def _compute_etwfe_effects_ols(self) -> None:
        ...
        self.att_event_time_ = self._apply_event_window(pd.DataFrame(rows))
        ...
        self.att_group_time_ = pd.DataFrame(group_time)
    ...
    def _compute_etwfe_effects_bayesian(self, hdi_prob: float = HDI_PROB) -> None:
        ...
        self.att_event_time_ = self._apply_event_window(
    ...
    def _event_study_axis_limits(
        self, att_et: pd.DataFrame
    ) -> tuple[np.ndarray, float]:
        """Tick positions and left edge of the pre-treatment shading.

        The omitted reference event time carries no estimate but is drawn as a
        marker, so it has to be included in both.

        Parameters
        ----------
        att_et : pd.DataFrame
            The event-time ATT table being plotted.

        Returns
        -------
        tuple[np.ndarray, float]
            Sorted tick positions and the smallest plotted event time.
        """
        event_times = [int(e) for e in att_et["event_time"].values]
        if self.estimator == "etwfe":
            event_times.append(int(self.reference_event_time))
        ticks = np.array(sorted(set(event_times)))
        return ticks, float(ticks.min())
    ...
    def _bayesian_plot(self, **kwargs: Any) -> tuple[plt.Figure, list[plt.Axes]]:
        ...
        # Mark the omitted reference event time (ETWFE only)
        ...
        ticks, event_min = self._event_study_axis_limits(att_et)
        if event_min < 0:
            ax.axvspan(
                event_min - 0.5,
                -0.5,
