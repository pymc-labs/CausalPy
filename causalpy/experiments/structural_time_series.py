...
class StructuralTimeSeries(BaseExperiment):
    ...
    This is a type of structural time series model, based on pre-specified
    bases (changepoints, Fourier, HSGP, etc).
    ...
    expt_type = "Basis Expansion Time Series"
    supports_ols = True
    supports_bayes = True
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(self.model, PyMCModel):
            # Coords for pre-intervention period
            # All PyMCModels will receive X as NumPy array.
            # BSTS will look for "datetime_index" in coords. Others will ignore it.
            pre_coords = {
                "coeffs": self.labels,
                "obs_ind": np.arange(self.pre_X.shape[0]),
                "datetime_index": self.datapre.index,  # Added for BSTS
            }
            # Determine X to pass: if pre_X has 0 columns (e.g. y ~ 1), pass None or empty array.
            # This depends on how BSTS's _prepare_time_and_exog_features handles it.
            # Assuming it handles None or 0-column array gracefully if self.labels is empty.
            X_fit = self.pre_X if self.pre_X.shape[1] > 0 else None

            self.model.fit(X=X_fit, y=self.pre_y, coords=pre_coords)
        ...
        X_score = self.pre_X if self.pre_X.shape[1] > 0 else None
        if isinstance(self.model, PyMCModel):
            # Coords for pre-intervention period (for scoring)
            pre_score_coords = {
                "coeffs": self.labels,  # Needed if model uses them for identifying X structure
                "datetime_index": self.datapre.index,  # For BSTS
                # obs_ind might not be needed by score/predict if model infers from X shape
            }
            self.score = self.model.score(
                X=X_score, y=self.pre_y, coords=pre_score_coords
            )
        elif isinstance(self.model, RegressorMixin):
            self.score = self.model.score(X=X_score, y=self.pre_y)
        else:
            raise ValueError("Model type not recognized")

        # get the model predictions of the observed (pre-intervention) data
        X_pre_predict = self.pre_X if self.pre_X.shape[1] > 0 else None
        if isinstance(self.model, PyMCModel):
            # Coords for pre-intervention period (for prediction)
            pre_pred_coords = {
                "coeffs": self.labels,
                "datetime_index": self.datapre.index,  # For BSTS
            }
        ...
        X_post_predict = self.post_X if self.post_X.shape[1] > 0 else None
        if isinstance(self.model, PyMCModel):
            # Coords for post-intervention period (for prediction)
            post_pred_coords = {
                "coeffs": self.labels,
