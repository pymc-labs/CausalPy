...
def _extract_hdi_bounds(
    ...
    if isinstance(hdi_result, xr.Dataset):
        ...
        lower = _as_float(hdi_data.sel(hdi="lower").values)
        upper = _as_float(hdi_data.sel(hdi="higher").values)
    else:
        lower = _as_float(hdi_result.sel(hdi="lower").values)
        upper = _as_float(hdi_result.sel(hdi="higher").values)
    return lower, upper
...
def _compute_tail_probabilities(
    ...
    if direction == "increase":
        return {"p_gt_0": _as_float((effect > 0).mean().values)}
    elif direction == "decrease":
        return {"p_lt_0": _as_float((effect < 0).mean().values)}
    else:  # two-sided
        p_gt = _as_float((effect > 0).mean().values)
        p_lt = _as_float((effect < 0).mean().values)
...
def _compute_rope_probability(
    ...
    if direction == "two-sided":
        return _as_float((np.abs(effect) > min_effect).mean().values)
    elif direction == "increase":
        return _as_float((effect > min_effect).mean().values)
    elif direction == "decrease":
        return _as_float((effect < -min_effect).mean().values)
...
def _compute_statistics_scalar(
    ...
    """
    stats = {
        "mean": _as_float(effect.mean(dim=["chain", "draw"]).values),
        "median": _as_float(effect.median(dim=["chain", "draw"]).values),
    }
...
def _compute_statistics(
    ...
    stats["avg"] = {
        "mean": _as_float(avg_effect.mean(dim=["chain", "draw"]).values),
        "median": _as_float(avg_effect.median(dim=["chain", "draw"]).values),
    }
    ...
    if isinstance(hdi_avg, xr.Dataset):
        ...
        stats["avg"]["hdi_lower"] = _as_float(hdi_data.sel(hdi="lower").values)
        stats["avg"]["hdi_upper"] = _as_float(hdi_data.sel(hdi="higher").values)
    else:
        # If it's a DataArray, extract directly
        stats["avg"]["hdi_lower"] = _as_float(hdi_avg.sel(hdi="lower").values)
        stats["avg"]["hdi_upper"] = _as_float(hdi_avg.sel(hdi="higher").values)

    # Tail probabilities for average
    if direction == "increase":
        stats["avg"]["p_gt_0"] = _as_float((avg_effect > 0).mean().values)
    elif direction == "decrease":
        stats["avg"]["p_lt_0"] = _as_float((avg_effect < 0).mean().values)
    else:  # two-sided
        p_gt = _as_float((avg_effect > 0).mean().values)
        p_lt = _as_float((avg_effect < 0).mean().values)
        p_two_sided = 2 * min(p_gt, p_lt)
    ...
    if cumulative:
        ...
        stats["cum"] = {
            "mean": _as_float(cum_final.mean(dim=["chain", "draw"]).values),
            "median": _as_float(cum_final.median(dim=["chain", "draw"]).values),
        }
        ...
        if isinstance(hdi_cum, xr.Dataset):
            ...
            stats["cum"]["hdi_lower"] = _as_float(hdi_cum_data.sel(hdi="lower").values)
            stats["cum"]["hdi_upper"] = _as_float(hdi_cum_data.sel(hdi="higher").values)
        else:
            stats["cum"]["hdi_lower"] = _as_float(hdi_cum.sel(hdi="lower").values)
            stats["cum"]["hdi_upper"] = _as_float(hdi_cum.sel(hdi="higher").values)

        # Tail probabilities for cumulative
        if direction == "increase":
            stats["cum"]["p_gt_0"] = _as_float((cum_final > 0).mean().values)
        elif direction == "decrease":
            stats["cum"]["p_lt_0"] = _as_float((cum_final < 0).mean().values)
        else:  # two-sided
            p_gt = _as_float((cum_final > 0).mean().values)
            p_lt = _as_float((cum_final < 0).mean().values)
            p_two_sided = 2 * min(p_gt, p_lt)
    ...
    if relative:
        ...
        stats["avg"]["relative_mean"] = _as_float(
        ...
        if isinstance(hdi_rel_avg, xr.Dataset):
            ...
            stats["avg"]["relative_hdi_lower"] = _as_float(
                hdi_rel_avg_data.sel(hdi="lower").values
            )
            stats["avg"]["relative_hdi_upper"] = _as_float(
        ...
        if cumulative:
            ...
            stats["cum"]["relative_mean"] = _as_float(
            ...
            if isinstance(hdi_rel_cum, xr.Dataset):
                ...
                stats["cum"]["relative_hdi_lower"] = _as_float(
                    hdi_rel_cum_data.sel(hdi="lower").values
                )
                stats["cum"]["relative_hdi_upper"] = _as_float(
