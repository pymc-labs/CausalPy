...
print(
    f"hier sample N={N} cities={len(city_codes)} countries={nC} | {dict(zip(ctry_codes, np.bincount(ctry_idx)))}"
)
...
with pm.Model(coords=coords) as m:
    ...
    pm.Potential(
        "lik",
        (
            pt.as_tensor_variable(w)
            * pm.logp(pm.Beta.dist(mu * phi, (1 - mu) * phi), y2)
        ).sum(),
    )
    idata = pm.sample(
        draws=900,
...
rhat = float(
    az.rhat(idata, var_names=["a", "tau", "bL", "bR", "phi", "sig_city"])
    .to_array()
    .max()
)
...
if nC > 1:
    cfe_s = (
        post["cfe"].stack(s=("chain", "draw")).transpose("s", "country").values
    )  # (S,nC)
...
def summ(s):
    return dict(
        ...
        hdi3=round(float(np.percentile(s, 3)), 4),
        hdi97=round(float(np.percentile(s, 97)), 4),
        p_negative=round(float((s < 0).mean()), 4),
    )


out = {
    "model": "hierarchical_beta_kernel_local_linear",
    "bandwidth": H,
    "N": N,
    "n_cities": int(len(city_codes)),
    "n_countries": int(nC),
    "divergences": div,
    "max_rhat": round(rhat, 4),
    "tau_logit": summ(tau_s),
    "tau_cri2_scale": summ(tau_cri2),
...
az.plot_posterior(tau_s, ref_val=0, hdi_prob=0.94, ax=ax[0])
ax[0].set_title("Discontinuity tau (logit scale)")
az.plot_posterior(tau_cri2, ref_val=0, hdi_prob=0.94, ax=ax[1])
ax[1].set_title("Discontinuity on cri2 scale")
for a_ in ax:
    a_.axvline(0, color="k", ls=":")
ax[1].axvline(-0.078, color="red", ls="--", label="paper -0.078")
ax[1].legend()
fig.suptitle(
    "Hierarchical Beta kernel-weighted RD: effect of salary raise on corruption risk"
)
fig.savefig("out/fig_hier_posterior.png", dpi=130, bbox_inches="tight")
plt.close(fig)
