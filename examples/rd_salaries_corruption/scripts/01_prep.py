...
cols = ['year','country','citycode','city_pop_new','salary_ppp','salary_group',
        'margin','uniques','non_uniques','cri','cri2','cri3',
        'corr_proc','corr_singleb'] \
...
print('mean salary_ppp just below (margin in [-0.05,0)):', fs.loc[(fs.margin>=-0.05)&(fs.margin<0),'salary_ppp'].mean())
print('mean salary_ppp just above (margin in [0,0.05)):', fs.loc[(fs.margin>=0)&(fs.margin<0.05),'salary_ppp'].mean())

# save compact analysis frames
keep = ['year','country','citycode','city_pop_new','salary_ppp','salary_group',
        'margin','uniques','non_uniques','cri','cri2','cri3','corr_proc','corr_singleb'] \
       + [f'ccode{i}' for i in range(1,12)] + [f'year_{i}' for i in range(1,16)]
