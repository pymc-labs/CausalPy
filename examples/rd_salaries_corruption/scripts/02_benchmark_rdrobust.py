...
def run(mask, covs, label):
    ...
    out = rdrobust(
        ...
        c=0,
        covs=cv.values,
        cluster=d["citycode"].values,
        all=True,
    )
    co = np.asarray(out.coef).flatten()
    se = np.asarray(out.se).flatten()
    pv = np.asarray(out.pv).flatten()
    h = float(np.asarray(out.bws).flatten()[0])
    Nh = int(np.asarray(out.N_h).sum())
    res = dict(
        label=label,
        n=int(len(d)),
        N_in_bw=Nh,
        ...
        se_robust=round(float(se[2]), 4),
        p_robust=round(float(pv[2]), 5),
    )
    print(label, "->", res)
    return res
...
results = []
results.append(
    run((df.uniques == 1) & (df.margin < 1), covars1, "T2c1_unique_thresholds")
)
results.append(
    run((df.non_uniques == 1) & (df.margin < 1), covars2, "T2c2_compound_thresholds")
)
