...
- Dependencies live in `pyproject.toml`; both `uv.lock` (canonical, default dev + CI) and `environment.yml` (conda alternative, generated by a prek hook) are derived from it — do not hand-edit either. The docs-only PyMC-Marketing transition snapshot is pinned separately in `docs/requirements.txt`.
